...
learn = tf.keras.models.load_model('./FER_ModifiedModel_balanced.h5')
...
path = './'
if args["save"]:
    out = cv2.VideoWriter(path + "liveoutput.avi",
...
while True:
    ...
    for coords in face_coord:
        ...
        if FRAMES % 100 == 0:
            prediction = np.argmax(learn.predict(np.expand_dims(
                np.expand_dims(cv2.resize(img_cp, (48, 48)), -1), 0)))
            print(learn.predict(np.expand_dims(np.expand_dims(cv2.resize(img_cp, (48, 48)), -1), 0)))
        cv2.rectangle(
            img=frame,
            pt1=(X_1, Y_1),
            pt2=(X_2, Y_2),
        ...
        if FRAMES % 100 == 0:
            print(prediction)
        cv2.putText(frame, str(classIndices[prediction]), (
